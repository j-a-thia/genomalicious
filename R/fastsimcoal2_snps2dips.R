...
#' }
fastsimcoal2_snps2dips <- function(arpFile){
  library(data.table)
  ...
  # Make locus names using the chromosome and in SNP number
  chrom_loci <- unlist(
  ...
  diploids <- lapply(1:numSamps, function(n.samp){
    ...
    # How many items per sampled genome
    ...
    samp.gts <- data.table(POP=paste0('pop_', n.samp)
                           , SAMPLE=paste0('ind_', n.samp, '_', 1:samp.size)
                           , as.data.table(samp.gts))
