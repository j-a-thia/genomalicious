...
#' a screeplot of eigenvalues (\code{'scree'}), or the cumulative explained
...
#' pca <- pca_DTgenos(pcaObj=datGt, scaling='patterson', popCol='POP')
#'
#' # Plot the PCA
#' pca_plot(pca)
#'
...
#'
#' @export
pca_plot <- function(pcaObj, type='scatter', axisIndex=NULL
                     , pops=NULL, plotColours=NULL, look='ggplot'){
  ...
  # Specify axes to plot for scree and cumulative variance plots
  # Assign colour if unspecified for scree and cumulative variance plots.
  ...
  } else{ plot.tab <- as.data.table(pcaObj) }
  ...
  if(type=='scatter'){
    ...
    if(is.null(pops)==TRUE){ gg <- gg + geom_point()
      ...
      gg <- gg + geom_point(aes(colour=POPS)) + scale_colour_manual(values=plotColours) + labs(colour=NULL)
    }
  }
