...
poolne_estim_input <- function(dat, pool.info, runID, mcmc.sampling=c(1000, 25, 5000, 20, 50, sample(as.integer(1000:9999),1))) {
  ...
  if(sum(c("POOL", "SAMPLE", "CHROM", "LOCUS", "AO", "RO") %in% colnames(dat))!= 6){
    stop("Argument dat needs the columns $POOL, $SAMPLE, $CHROM, $LOCUS, $AO, and $RO.")
  }
  ...
  # Code
  # --------------------------------------------+
  # Make depth equal to RO + AO
  dat$DP <- dat$RO + dat$AO
