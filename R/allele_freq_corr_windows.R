...
allele_freq_corr_windows <- function(
  ...
  for(chrom in uniq.chrom){
    ...
    int.tab <- data.table(START=seq(1, seq.max, windowSize)) %>%
    ...
    for(j in 1:max(int.tab$WINDOW)){
      uniq.loc.win <- loc.tab %>%
        .[POS>=int.tab$START[j] & POS<=int.tab$END[j]] %>%
        .[['LOCUS']]
      ...
      loc.dist.win <- dat.chrom[LOCUS %in% work.loc.win] %>%
        ...
        data.table(CHROM=chrom, WINDOW=j, .)
      ...
      dist.cor.list[[chrom]][[j]] <- left_join(
