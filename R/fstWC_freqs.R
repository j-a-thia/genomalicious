...
#'
#' @export
fstWC_freqs <- function(
  freqMat, sampMat, doPairs=FALSE, num.cores=1,
  bootCI=FALSE, boots=100,
  doDist=FALSE, perLocus=FALSE){
  ...
  if(doPairs==FALSE){
    ...
    lociVar <- fstWC_varcomps(dat=freqMat, input_type='freqs', samp_size=sampMat, num.cores=num.cores)
    ...
    # Bootstrap theta
    if(bootCI==TRUE){
      fst.boot <- fstWC_boot(dat=freqMat, samp_size=sampMat, input_type='freqs', num.cores=num.cores)
  ...
  if(doPairs==TRUE){
    ...
    pairList <- apply(pairCombos, 2, function(X){
      ...
      # Calculate variance per locus
      lociVar <- fstWC_varcomps(dat=freqMat[X,], input_type='freqs', samp_size=sampMat[X,], num.cores=num.cores)
      ...
      # Bootstrap theta
      if(bootCI==TRUE){
        fst.boot <- fstWC_boot(dat=freqMat[X,], input_type='freqs', samp_size=sampMat[X,], boots=boots, num.cores=num.cores)
