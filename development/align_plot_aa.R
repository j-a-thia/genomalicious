...
#' the IUPAC conventions and '?' or '-'.
#'
#' @param aa_colours Character: named vector colour values for small nonpolar ('sn'),
#' hydrophobic ('h'), polar ('p'), negatively charged ('nc'), and positively charged ('pn')
#' amino acids. For example, \code{c('sn'='organge','h'='green','p'='magenta','nc'='red','pc'='blue')}.
#' Default is NA, which will produce automated colours.
#'
#' @param other_colours Character: named vector colour values for '?'
#' values. For example, \code{c('?'='black')}. Default is NA, which will produce automated colours.
#'
#' @param gap_colour Chracter: named vector colour for gaps. For examples,
#' \code{c('-'='white')}. Default is NA, which will produce automated colours.
#'
...
#'
#' @param show_aa Logical: whether the values of bases should be plotted.
#' Default is TRUE.
#'
...
#'    aa_colours=c(`A`='#ce0073',`T`='#e46adf',`G`='royalblue',`C`='#08c7e0'),
#'    pos_vec=200:250,
#'    show_aa=FALSE
#' )
#'
...
align_plot_aa <- function(
    ...
    border_colour=NA, pos_vec=NULL, show_aa=TRUE, base_size=3, show_legend=FALSE){
